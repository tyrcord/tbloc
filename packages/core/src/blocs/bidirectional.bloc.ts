...
import {
  BidirectionalBlocDelegate
} from '../types/bidirectional-bloc-delegate.type';
...
export abstract class BidirectionalBloc<T extends BlocEvent, K = {}>
  extends Bloc<K, keyof BidirectionalBlocDelegate<T, K>> {

  protected abstract mapEventToState(event: T, currentState: K): (
  ...
  constructor(initialState: K) {
    ...
    this.eventSubscription = this.eventController.subscribe((candidateEvent: T) => {
      ...
      let event = delegateEvent || candidateEvent;
      ...
      promise.then((nextState: K) => {
        const { meta } = event;
        const updateStrategy = meta && meta.updateStrategy ?
          meta.updateStrategy : this.updateStrategy;
        ...
        if (this.delegateRespondsToMethod('blocDidProcessEvent')) {
          // @ts-ignore -- `delegate` is safe here,
          // thanks to`delegateRespondsToMethod`
          this.delegate.blocDidProcessEvent(
            this,
            event,
            nextState,
          );
